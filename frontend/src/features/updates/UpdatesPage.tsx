...
export function UpdatesPage() {
  ...
  return <section className="page"><PageHeader title="更新记录" description="记录每次更新的旧/新镜像、Digest、结果和错误，为后续回滚保留依据。" action={<Button icon={<ReloadOutlined />} onClick={() => void refresh()} loading={loading}>刷新</Button>} />{error ? <Alert className="inline-alert" type="warning" showIcon message="更新记录不可用" description={error.message} /> : null}<Table<UpdateRecord> className="dense-table" rowKey="id" size="small" columns={columns} dataSource={data} loading={loading} scroll={{ x: 1320 }} pagination={{ pageSize: 20, showTotal: (total) => `共 ${total} 条记录` }} /></section>
}
