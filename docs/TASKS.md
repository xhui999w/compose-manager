...
- [x] 搜索、状态筛选、展开容器行。
- [x] 响应式紧凑项目列表：宽屏双栏、窄屏单栏，全局排序后按页面顺序排列，并保留容器展开与全部项目操作。
- [x] 状态色和行操作；访问保留为行按钮，首页不再占用容器计数、镜像更新、更新策略或内/外网独立列。
- [x] Docker 不可用、加载和空状态。
...
- [x] 删除确认与执行前服务端二次检查。
- [x] 支持勾选、全选当前筛选中的可删除镜像和批量删除；每个镜像仍由服务端逐项复核引用。
...
- [x] 按使用反馈移除容器卡片中的 Compose 标签与端口/创建时间行，进一步压缩卡片高度。
- [~] pull → up 流水线（已完成；基于 healthcheck 的等待/失败回退待真实主机集成验证）。
- [x] 旧/新镜像与 digest、结果、错误记录。
- [x] 更新记录页面。
- [x] 左侧更新进度页面；更新任务异步执行并实时展示下载镜像、重建容器、状态检查、结果和受限详细输出。
- [x] 设置页支持可选镜像检查代理与 2～60 分钟更新阶段超时；默认更新超时提高到 15 分钟，保存后对新任务立即生效。
- [x] 更新成功后立即清除对应服务的“有更新”缓存；容器页停留期间自动等待任务结束并刷新状态。
