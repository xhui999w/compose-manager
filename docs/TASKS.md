...
- [x] 搜索、状态筛选、展开容器行。
- [x] 响应式紧凑项目列表：宽屏双栏、窄屏单栏，全局排序后按页面顺序排列；排序方式以可见按钮直接呈现，并保留容器展开与全部项目操作。
...
## Phase 9 — Compose 工作区与项目创建

- [x] 新增受允许根目录约束的项目文件页面，支持文件夹浏览与创建。
- [x] 常用文本配置文件可查看和编辑，保存前校验基准 SHA、自动备份并原子替换。
- [x] Compose 首页提供“新建 Compose”，支持选择根目录、相对目录、源码及创建后启动。
- [x] 创建前执行 YAML 与 `docker compose config` 校验，禁止覆盖已有项目目录。
- [x] 路径穿越和符号链接逃逸测试；所有工作区 API 保持登录与 CSRF 保护。

验收：浏览器不能访问允许根目录之外的文件；创建失败不留下 Compose 文件；配置保存在 NAS 服务端并可由其他已登录电脑共同使用。

## Phase 10 — Compose 卡片首页

- [x] Compose 首页改为高密度响应式项目卡片：宽屏六列，中等宽度自动降为五/四/三/二列，窄屏单列。
- [x] 卡片保留状态、来源、CPU、内存、更新提示和全部项目操作。
- [x] 访问与重新发现归入“更多”，降低常用操作区宽度且不移除能力。
- [x] 项目展开后横跨整行显示容器明细，避免表格在单张卡片内过度压缩。
- [x] 浅色、深色与舒适密度样式同步适配。
- [x] 精简侧栏：移除独立容器入口，将“项目文件”改为“Docker 根目录”，“镜像”改为“镜像管理”。

验收：1920px 及以上宽屏显示六列；卡片无按钮溢出；筛选、排序、展开和创建入口保持可用。
