...
- 用户确认后创建内存更新任务并立即返回任务 ID；前端“更新进度”页面每 1.5 秒轮询任务列表，展示 pulling、applying、checking、completed/failed 阶段。刷新网页不会丢失正在运行的任务；服务重启时任务随进程终止。
- Compose 子进程输出按行限量收集，移除 ANSI 控制符并遮蔽常见凭据键；浏览器只读取处理后的输出，不接触命令参数或 Docker Socket。
